const {loginPublic, scrollElementIntoCenter, select2MultiSelect, formSelect, select2SingleSelect, waitLineupReady, waitPhoveaNotBusy, checkScoreColLoaded, waitTdpNotBusy} = require("./utils/utils");
...
it('ordino video', function() {
    ...
    cy.intercept('https://cdn.jsdelivr.net/npm/cookie-bar/lang/en.html').as('cookie-bar');
    // Login
    ...
    checkScoreColLoaded(0, ['HCC1954', 'Relative Copy Number'], 9);
    ...
    cy.get('[data-testid=primary-dialog-button]').click();
    checkScoreColLoaded(0, ['HCC1954', 'Normalized Gene Expression (TPM Values)'], 10)
    ...
    cy.get('[data-testid=primary-dialog-button]').click();
    checkScoreColLoaded(0, ['HCC1954', 'DRIVE RSA (ER McDonald III et al., Cell, 2017)'], 11)
    ...
    cy.get('[data-testid=primary-dialog-button]').click();
    checkScoreColLoaded(0, ['Tumor Type', 'breast carcinoma', 'Normalized Gene Expression (TPM Values)'], 12)
    ...
    cy.get('[data-testid=primary-dialog-button]').click();
    checkScoreColLoaded(0, ['Relative Copy Number', 'Boxplot'], 13)
    ...
    cy.get('[data-testid=primary-dialog-button]').click();
    checkScoreColLoaded(0, ['Frequency > 4'], 14)
    ...
    cy.get('[data-testid=primary-dialog-button]').click();
    checkScoreColLoaded(1, ['BRCA1', 'AA Mutated'], 10)
    checkScoreColLoaded(1, ['BRCA2', 'AA Mutated'], 10)
